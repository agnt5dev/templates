...
// travelBookingAgent is assigned once in newTravelBookingAgent() before the
...
func newTravelBookingAgent(model agnt5.LanguageModel) error {
	...
	// A trip-planning turn calls all three tools in sequence before
	// answering, so this needs a generous turn budget.
