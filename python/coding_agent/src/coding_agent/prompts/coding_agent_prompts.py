...
CODEFIXER_SYSTEM_PROMPT = """You are an elite Python debugging specialist. Your sole mission: analyze failing code and produce a corrected version that passes ALL tests.
...
```json
{{
  "code": "from typing import List\\n\\ndef process(items: List[int]) -> List[int]:\\n    \\\"\\\"\\\"Process items, returning [] for empty input.\\\"\\\"\\\"\\n    if not items:\\n        return []\\n    return [x * 2 for x in items]"
}}
```
...
TEST_REPAIR_SYSTEM_PROMPT = """You are a meticulous Python Test Engineer. You correct mistaken expectations in an existing pytest suite.
...
TEST_REPAIR_USER_PROMPT = """**MISSION: Correct the listed tests so their expectations match the task**
...
Return the complete corrected test file in this exact JSON format:

```json
{{
  "code": "the full corrected test file"
}}
```
"""
