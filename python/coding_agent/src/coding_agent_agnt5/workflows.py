...
from coding_agent_agnt5.models import WorkflowResult
...
async def coding_agent_workflow(
    ...
    try:
        ...
        plan = await ctx.task(planner_node, task_description)
        ...
        for iteration in range(max_retries):
            ...
            if iteration == 0:
                ctx.logger.info("⚡ Parallel generation: code + tests")

                code_result, test_result = await ctx.parallel(
                    ctx.task(
                        code_generator_node,
            ...
            if not sync_result.success:
                ...
                return WorkflowResult(
                    ...
                    error=f"Code sync failed: {sync_result.message}",
                    code=generated_code,
            ...
            if next_action == "success":
                ...
                ctx.logger.warning(
                    f"⚠️ Tests failed - retrying ({retries}/{max_retries})"
                )
                continue

            else:
                ctx.logger.error(f"❌ Unexpected next_action: {next_action}")
                return WorkflowResult(
                    success=False,
                    task=task_description,
                    iterations=iteration + 1,
                    error=f"Unexpected next_action: {next_action}. Error: {error_logs}",
                    code=generated_code,
                    tests=generated_tests,
                    sandbox_id=sandbox_id,
                )
